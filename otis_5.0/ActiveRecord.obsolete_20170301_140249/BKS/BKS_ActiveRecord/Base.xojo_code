...
Protected Class Base
	#tag Method, Flags = &h0
		Sub ChangeMySavedValue(sFieldName as String, vValue as Variant)
		  ...
		  da1.ChangeSavedValue(self,sFieldName,vValue)
	...
	#tag Method, Flags = &h0
		Sub Delete()
		  ...
		  dim adp as BKS_ActiveRecord.DatabaseAdapter = GetDatabaseAdapter
	...
	#tag Method, Flags = &h0
		Function GetMyFieldValues(UnModifiedFields as Boolean = False) As JSONItem
		  ...
		  oJSON = da1.GetFieldValPairs(self,UnModifiedFields)
		  Return oJSON
		End Function
	#tag EndMethod
		  ...
		  for each oFieldInfo as BKS_ActiveRecord.P.FieldInfo in oTableInfo.aroField
		    ...
		    if vProperty.Type=Variant.TypeString then
		      ...
		      if StrComp(vProperty.StringValue, vSavedValue.StringValue, 0)<>0 then
		        bModified = true
		        ModifiedFields.Append(oFieldInfo.sFieldName)
		        'exit for
		      end if
		    else
	...
	#tag Method, Flags = &h0
		Function Load(iRecordID as Int64) As boolean
		  ...
		  dim ada as BKS_ActiveRecord.DatabaseAdapter = GetDatabaseAdapter
