...
Inherits DataFile.ActiveRecordBase
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as Integer) As DataFile.tbl_inv_ex
		  //Usage:
		  //dim tbl_inv_ex as DataFile.tbl_inv_ex = DataFile.tbl_inv_ex.FindByID( id )
		  dim s as string
		  s = "Select * from tbl_inv_ex WHERE pkid = " + str(id)
		  ...
		  dim tbl_inv_ex as new DataFile.tbl_inv_ex
		  tbl_inv_ex.ReadRecord(rs)
		  return tbl_inv_ex
	...
	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_inv_ex()
		  ...
		  dim aro() as DataFile.tbl_inv_ex
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_inv_ex
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_inv_ex()
		  ...
		  ars.append DataFile.tbl_inv_ex.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_inv_ex.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
