#tag Class
Protected Class tbl_contact_method_linking
Inherits DataFile.ActiveRecordBase
	#tag Method, Flags = &h0
		Shared Function BaseSQL(bAsCount as Boolean = false) As String
		  ...
		  ars.Append "From tbl_contact_method_linking"
	...
	#tag Method, Flags = &h0
		Shared Function FindByID(id as Integer) As DataFile.tbl_contact_method_linking
		  //Usage:
		  //dim tbl_contact_method_linking as DataFile.tbl_contact_method_linking = DataFile.tbl_contact_method_linking.FindByID( id )
		  dim s as string
		  s = "Select * from tbl_contact_method_linking WHERE pkid = " + str(id)
		  ...
		  dim tbl_contact_method_linking as new DataFile.tbl_contact_method_linking
		  tbl_contact_method_linking.ReadRecord(rs)
		  return tbl_contact_method_linking
		  
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Shared Function List(stmt as PreparedSQLStatement) As DataFile.tbl_contact_method_linking()
		  ...
		  dim aro() as DataFile.tbl_contact_method_linking
		  ...
		  while rs.eof = false
		    dim oRecord as new DataFile.tbl_contact_method_linking
	...
	#tag Method, Flags = &h0
		Shared Function List(sCriteria as string = "", sOrder as string = "", iOffset as Integer = -1) As DataFile.tbl_contact_method_linking()
		  ...
		  ars.append DataFile.tbl_contact_method_linking.BaseSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListCount(sCriteria as string = "") As Integer
		  ...
		  ars.append DataFile.tbl_contact_method_linking.BaseSQL(True)
		  if sCriteria<>"" then
		    ars.append "WHERE " + sCriteria
		  end if
		  dim s as string = ars.JoinSQL
	...
	#tag Method, Flags = &h0
		Shared Function ListGrouped(sCriteria as string = "", sOrder as string = "", sGroupBy as String = "") As Dictionary
		  ...
		  dim oRecordList() as DataFile.tbl_contact_method_linking
		  ...
		  oRecordList() = DataFile.tbl_contact_method_linking.List(sCriteria, sOrder)
		  ...
		  For Each oRecord as DataFile.tbl_contact_method_linking In oRecordList()
		    ...
		    For idx1 as integer = 0 To sGroupByList.Ubound
		      ...
		      If sArray.IndexOf( sGroupValue ) > -1 THen
		        ...
		        Elseif jsCurrent(n3).Value( sGroupValue ) IsA DataFile.tbl_contact_method_linking Then
		        ...
		        Else
		          ...
		          Try
		            ...
		            dim oRecords() as DataFile.tbl_contact_method_linking
