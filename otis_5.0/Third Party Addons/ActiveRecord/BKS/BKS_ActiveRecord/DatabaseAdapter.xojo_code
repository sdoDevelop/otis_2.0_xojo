...
Protected Class DatabaseAdapter
	...
	#tag Method, Flags = &h0
		Sub ChangeSavedValue(oRecord as BKS_ActiveRecord.Base, sFieldName as String, vValue as Variant)
		  dim oTableInfo as BKS_ActiveRecord.P.TableInfo
		  
		  dim dictFieldValue as Dictionary
		  
		  dim oFieldVal as New JSONItem
		  ...
		  for each oField as BKS_ActiveRecord.P.FieldInfo in oTableInfo.aroField
		    ...
		    If oField.sFieldName = sFieldName Then
		      dim pi as Introspection.PropertyInfo = oField.piFieldProperty
		      pi.Value(oRecord) = vValue
		    End If
	...
	#tag Method, Flags = &h0
		Function GetFieldValPairs(oRecord as BKS_ActiveRecord.Base,UnModifiedFields as Boolean = False) As JSONItem
		  dim oTableInfo as BKS_ActiveRecord.P.TableInfo
		  ...
		  for each oField as BKS_ActiveRecord.P.FieldInfo in oTableInfo.aroField
		    ...
		    'continue
		    'end if
		    // Check if this is a modified field and add if it is
		    If oRecord.ModifiedFields.IndexOf(oField.sFieldName) > -1 Or UnModifiedFields Then
		  ...
		  for i as integer = 0 to aroField.Ubound
		    ...
		    'oFieldVal.Value(pi.Name) = v
		    oFieldVal.Value(arsField(i)) = v
