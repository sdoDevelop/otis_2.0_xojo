...
Inherits Window
	...
	#tag Method, Flags = &h0
		Sub LoadWindowPrefs()
		  dim temp as string = Preferences.WindowValue(GetclassName,me)
		  me.left = val(nthField(temp, ";", 1))
		  me.top = Max(val(nthField(temp, ";", 2)), screen(0).AvailableTop)
		  me.height = val(nthField(temp, ";", 3))
		  me.width = val(nthField(temp, ";", 4))
		  
		  #if TargetWin32 then
		    if me.left > screen(0).Width then
		      me.left = 150
		    end
		    
		    if me.top > screen(0).Height then
		      me.left = 150
		    end
	...
	#tag Method, Flags = &h0
		Sub SaveWindowPrefs()
		  Preferences.WindowValue(GetClassName)= me
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub SetWindowFileBKS(F As FolderItem)
		  xAssociatedFile = F
		End Sub
	#tag EndMethod
	...
	#tag Property, Flags = &h21
		Private xAssociatedFile As FolderItem
	#tag EndProperty
